...
class Kinetics(data.Dataset):
    def __init__(self, video_path, of_path, slice_num, batch_size, frame_num=9, img_size=112, overlap_rate=0.25):
        self.samples = []
    ...
    def __getitem__(self, idx):
        ...
        ofs = torch.zeros(self.slice_num, self.batch_size, length, 2, self.img_size, self.img_size)
            ...
            for i in range(v_length):
                if i % self.space == self.space - 1:
                    try:
                        ...
                        of_img = Image.open(self.of_path + '/' + video_name + '/' + frames[i])
                        imgs.append(img)
                        of_imgs.append(of_img)
                    except:
                        pass
            ...
            for i in range(self.slice_num):
                ...
                for j in range(length):
                    ...
                    if n_frame < len(imgs):
                        ...
                        of_sample.append(of_imgs[n_frame])
                try:
                    while len(sample) < length * (i + 1):
                        sample.append(sample[-1])
                    while len(of_sample) < length * (i + 1):
                        of_sample.append(of_sample[-1])
                except:
                    print(video_name)

            input = torch.zeros(self.slice_num, length, 3, self.img_size, self.img_size)
            of = torch.zeros(self.slice_num, length, 2, self.img_size, self.img_size)
            sample_with_of_sample = []
            sample_with_of_sample.extend(sample)
            sample_with_of_sample.extend(of_sample)
            sample_with_of_sample = self.argumentation(sample_with_of_sample)
            sample = sample_with_of_sample[: len(sample_with_of_sample) // 2]
            of_sample = sample_with_of_sample[len(sample_with_of_sample) // 2:]

            for i in range(self.slice_num):
                for j in range(length):
                    img = sample[i * length + j].resize((self.img_size, self.img_size))
                    of_img = of_sample[i * length + j].resize((self.img_size, self.img_size))
                    transformed_img = self.transform(img)
                    transformed_of_img = self.transform(of_img)
                    img.close()
                    of_img.close()
                    input[i, j, :, :, :] = transformed_img
                    of[i, j, :, :, :] = transformed_of_img[0:2]
        ...
        return inputs, ofs, 0, overlap_frame_num, actions
