...
parser.add_argument('--video_path', type=str, default='/Disk1/kinetics_scaled', help='video path')
...
parser.add_argument('--dropout', type=list, default=[0.0, 0.0], help='dropout')
parser.add_argument('--weight_decay', type=float, default=1e-5, help='weight decay')
parser.add_argument('--saveInter', type=int, default=1, help='how many epoch to save once')
parser.add_argument('--q', type=float, default=0.5, help='propabaility of detachout')
parser.add_argument('--adjust_q', type=bool, default=True, help='whether adjust q automatically')
...
parser.add_argument('--lambdaa', type=float, default=1e3, help='weight of the overlap coherence loss')
...
model = actionModel(opt.class_num, batch_norm=True, dropout=opt.dropout, q=opt.q, image_size=opt.img_size, syn_bn=opt.syn_bn, test_scheme=2)
print(model)
if opt.syn_bn:
...
def loss_mse_func(temp_maps, ofs):
    loss = torch.zeros(1).cuda()
    for i in range(len(temp_maps)):
        gt = torch.nn.functional.interpolate(ofs, size=(2, temp_maps[i].shape[3], temp_maps[i].shape[4]))
        loss += nn.MSELoss()(temp_maps[i], gt)
    return loss
...
KineticsLoader = torch.utils.data.DataLoader(
    Kinetic_train_dataset.Kinetics(video_path=opt.video_path + '/train_frames',
                                   of_path=opt.video_path + '/train_ofs',
                                   frame_num=opt.frame_num, batch_size=opt.batch_size, img_size=opt.img_size,
                                   slice_num=opt.slice_num, overlap_rate=opt.overlap_rate),
...
for epoch in range(resume_epoch, opt.EPOCH):
    ...
    for step, (x, of, _, overlap_frame_num, action) in enumerate(KineticsLoader):  # gives batch data
        if opt.train:
            ...
            c = [Variable(torch.from_numpy(np.zeros((x.shape[1], model.module.channels[layer + 1], int(model.module.input_size[layer]), int(model.module.input_size[layer]))))).cuda().float() for layer in range(model.module.RNN_layer)]
            c_single = [Variable(torch.from_numpy(np.zeros((x.shape[1], model.module.channels[layer + 1], int(model.module.input_size[layer]), int(model.module.input_size[layer]))))).cuda().float() for layer in range(model.module.RNN_layer)]
            for slice in range(x.shape[0]):
                b_x = Variable(x[slice]).cuda()
                b_of = Variable(of[slice]).cuda()
                b_action = Variable(action[slice]).cuda()

                out, out_beforeMerge, data, data_beforeMerge, temp_maps, c, c_single = model(b_x.float(), c, c_single)  # rnn output
                ...
                loss_single_spatial = loss_classification_func(data, b_action[:, -1].long())
                loss_single_temp = loss_mse_func(temp_maps, b_of)
                loss = loss_classification + opt.lambdaa * loss_coherence
                loss += loss_single_spatial + loss_single_temp
                ...
                if opt.adjust_q:
                    q = adjust_q(loss_classification, loss_single_spatial)
                    for cell in model.module.RNN:
                        cell.q = q
            ...
            print("Epoch: " + str(epoch) + " step: " + str(step+resume_step) + " Loss: " + str(loss_classification.data.cpu().numpy()) + " Loss_spatial: " + str(loss_single_spatial.data.cpu().numpy()) + " Loss_temporal: " + str(loss_single_temp.data.cpu().numpy()) + " Loss_coherence: " + str(loss_coherence.data.cpu().numpy()) + " mAP: " + str(mAPs)[0:7] + " acc: " + str(acc)[0:7])
        ...
        if test:
            ...
            with torch.no_grad():
                ...
                for step_test, (x, _, _, action) in tqdm(enumerate(Loader_test)):  # gives batch data
                    ...
                    c_single = [Variable(torch.from_numpy(np.zeros((len(b_x), model.module.channels[layer + 1],
                                                             model.module.input_size[layer],
                                                             model.module.input_size[layer])))).cuda().float() for layer
                         in
                         range(model.module.RNN_layer)]
                    out, _, data, _, _, _, _ = model(b_x.float(), c, c_single)  # rnn output
                    loss = loss_classification_func(out, b_action[:, -1].long()) + \
                           loss_classification_func(data, b_action[:, -1].long())
    ...
    # if opt.adjust_q:
    #     q = adjust_q(loss_classification, loss_sigle_spatial)
    #     for cell in model.RNN:
    #         cell.q = q
